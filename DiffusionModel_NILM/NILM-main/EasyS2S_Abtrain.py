...
def get_arguments():
    ...
    parser.add_argument('--save_dir',
                        type=str,
                        default=f'./models/EasyS2SAb/{datasetName}',
...
def robust_signal_decay_loss(y_true, y_pred):
    L = tf.shape(y_true)[1]
    l_range = tf.range(1, L + 1, dtype=tf.float32)
    weights = tf.pow(l_range, -0.5)
...
def combined_loss(y_true, y_pred, alpha=1, beta=0.1):
    ...
    # return alpha * tf.reduce_mean(robust_loss,1)
...
if originModel:
    ...
    #cost = tf.reduce_mean(tf.reduce_mean(tf.abs(y - y_), 1))
...
if not args.transfer_model and args.transfer_cnn:
    save_path = args.save_dir + '/easy1_' + appliance_name + '_transf_' + args.cnn + '_abloss_model'
else:
    if originModel:
        save_path = args.save_dir + f'/easy{TrainNum}_{datasetName}' + appliance_name + f'{TrainPercent}_maeloss_model'
    else:
        # save_path = args.save_dir + f'/easy{TrainNum}_{datasetName}' + appliance_name + 'CombineEight5_pointnet_model'
        save_path = args.save_dir + f'/easy{TrainNum}_{datasetName}' + appliance_name + f'Combine{TrainPercent}_maeloss_model'
...
loss_data = pd.DataFrame({f'{applianceName}train_loss': train_loss, f'{applianceName}val_loss': val_loss})
loss_data.to_csv(f'{applianceName}mseloss_data.csv', index=False)
