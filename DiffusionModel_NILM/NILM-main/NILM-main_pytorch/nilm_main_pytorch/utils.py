...
def resolve_path(path_str: str | Path) -> Path:
    """Resolve relative to NILM-main_pytorch/, then NILM-main/, then repo root."""
    p = Path(path_str)
    if p.is_absolute():
        return p.resolve()

    for base in (PYTORCH_ROOT, NILM_MAIN_DIR, REPO_ROOT):
        candidate = (base / p).resolve()
        if candidate.exists():
            return candidate
    return (PYTORCH_ROOT / p).resolve()
...
def merge_cli_config(
    ...
    *,
    ...
    out["data"]["data_root"] = str(resolve_path(out["data"]["data_root"]))
